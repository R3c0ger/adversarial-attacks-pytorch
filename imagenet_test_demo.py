...
device = "cpu"
idx = 1
atk = torchattacks.PGD(model, eps=16/255, alpha=2/225, steps=10, random_start=True)

pdg_image = atk(images,labels)
#

pdg_pre = get_pred(model, pdg_image[idx:idx+1], device)
print("PDG attack: original label", idx2label[labels[idx:idx+1].item()], " pred label", idx2label[pdg_pre.item()])
#
imshow(pdg_image[idx:idx+1], title="PDG True:%s, Pre:%s"%(idx2label[labels[idx:idx+1].item()],  idx2label[pdg_pre.item()]))